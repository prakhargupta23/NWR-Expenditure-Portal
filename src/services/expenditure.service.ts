...
const documentKeys = {
  "ReceiptNote": {
    description: "Please extract the following specific fields from the provided json document and return the result as a JSON object using the exact key names listed: R/Note-No., Vendor Code, Supplier Name, Supplier Address, PO/AT No., PL No., R.O.No., R.O.Date, RN Quantity, Rate, Value, P.O.Sr.No.,Terms of Delivery, Freight, Inspection agency, IC no., dated, Consignee, Challan/invoice no., Date(Challan Date), Due Date of Delivery, Actual Date of Supply, Qty. Invoiced, Qty. Received, Qty. Accepted, and Qty. Rejected. Ensure each of these keys is assigned a corresponding value from the document. If any value is missing or cannot be identified, assign it a value of null. The output should contain only the extracted values in a properly structured JSON format without any explanation, summary, or extra content. Return all the values in string format.(dated comes after Gate/Challan Registration No and the value is below the text dated in next line)(if 'Inspection agency' is 'CONSG' then put its value as 'Consignee')(Convert dated,Date(Challan Date),R.O.Date into dd/mm/yy format)(Convert the values of 'Rate' and 'Value' into exact digits by removing any commas and currency symbols (like 'Rs' or 'INR') at the beginning and remove any decimal portion. Ensure the result is stored as a string.)(In the fields 'Qty. Invoiced' and 'Qty. Received', extract only the numeric values, remove any decimal portion, and ensure the final result is stored as a string.)(remove any decimal portion), Only include decimal portion in Rate"
  },
  "TaxInvoice": {
