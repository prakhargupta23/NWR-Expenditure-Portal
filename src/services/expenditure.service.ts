...
const documentKeys = {
  ...
  "TaxInvoice": {
    description: "Please extract the following specific fields from the provided json document of a tax invoice and return the result as a JSON object using the exact key names listed: Supplier Name, Supplier Address, GST No., Supplier PAN, CIN, Invoice No., Date, Qty, Rate, Freight Charges, GST Amount, Total Sales Amount(after gst addition), Destination, Dispatched through, e-Way Bill no., Bill of Landing/LR-RR No., and HSN Code. Ensure each of these keys is assigned a corresponding value from the document. If any value is missing or cannot be identified, assign it a value of null. The output should contain only the extracted values in a properly structured JSON format without any explanation, summary, or extra content. Return all the values in string format. Return all the values in string format.Return any date in dd/mm/yy format in string and convert it if it is not in that format. "
  },
  "GSTInvoice": {
